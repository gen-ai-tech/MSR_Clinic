...
export default function Navbar({ onBooking }) {
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 right-0 h-full w-72 max-w-[85vw] z-[60] bg-[#0F172A] border-l border-white/10 shadow-2xl flex flex-col transition-transform duration-400 ease-in-out lg:hidden ${
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-6 flex flex-col gap-1" aria-label="Mobile navigation">
          {navLinks.map((l) => (
            <a
              ...
              href={l.href}
              onClick={() => setMobileOpen(false)}
              className="text-slate-300 hover:text-white hover:bg-white/8 font-medium py-3.5 px-4 rounded-xl transition-all text-sm flex items-center gap-3 group"
            >
        ...
        <div className="px-4 pb-8 pt-4 border-t border-white/10">
          <a
            href="#appointment"
            onClick={() => setMobileOpen(false)}
            className="btn-shimmer text-white font-bold py-4 rounded-xl text-sm flex items-center justify-center gap-2 w-full shadow-lg"
          >
            <CalendarCheck className="w-4 h-4" />
            Book Appointment
          </a>
